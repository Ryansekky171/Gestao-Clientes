...
<div class="max-w-lg mx-auto bg-white p-6 rounded-xl shadow">
    <h2 class="text-2xl font-bold mb-4">Nova Conta Bancária</h2>
    ...
    <form action="{{ route('contas.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="banco" value="{{ old('banco') }}" class="w-full border rounded px-3 py-2" required>
            @error('banco') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block font-medium">Agência</label>
            <input type="text" name="agencia" value="{{ old('agencia') }}" class="w-full border rounded px-3 py-2">
            @error('agencia') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block font-medium">Conta</label>
            <input type="text" name="conta" value="{{ old('conta') }}" class="w-full border rounded px-3 py-2" required>
        ...
        <div>
            ...
            <select name="tipo" class="w-full border rounded px-3 py-2" required>
                <option value="corrente" {{ old('tipo') == 'corrente' ? 'selected' : '' }}>Corrente</option>
